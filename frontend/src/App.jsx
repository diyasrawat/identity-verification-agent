  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        {/* File selector — shown on Verification, Address, Fraud, Underwriter tabs */}
        ...
        {/* Loading */}
        {loading && (
          <div className="text-sm text-indigo-600 font-medium animate-pulse mb-6">
            ...
            {result && !loading && (
              <>
                ...
                {result.confidence_score && (
                  <div className="bg-white border border-gray-200 rounded-xl p-5">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span
                        className={`font-bold text-lg ${
                          result.confidence_score.color === "green"
                            ? "text-green-600"
                            : result.confidence_score.color === "yellow"
                            ? "text-yellow-600"
                            : "text-red-600"
                        }`}
                      >
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      {result.confidence_score.label} · Critical checks (DOB, PAN
                      format, Aadhaar) weighted 2×
                    </p>
                  </div>
                )}
                ...
                {result.reasoning_result?.agent_note && (
                  <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="font-bold text-indigo-900">Agent Reasoning</span>
                      <span
                        className={`ml-auto px-3 py-1 rounded-full text-xs font-bold ${
                          result.reasoning_result.recommendation === "PROCEED"
                            ? "bg-green-100 text-green-800"
                            : result.reasoning_result.recommendation === "REJECT"
                    ...
                    <p className="text-xs text-indigo-600">
                      ...
                      {result.reasoning_result.reasoning}
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-6 text-sm text-gray-600 bg-white rounded-xl border border-gray-200 px-6 py-3">
                  ...
                  <span className="text-gray-300">|</span>
                  <span>
                    <span className="font-bold text-red-600">{hardFails + softFails}</span> Failed
        ...
        {activeTab === "address" && (
          <div>
            ...
            {result && !loading && (
              <AddressPanel addressResult={result.address_result} />
            )}
          </div>
        )}
        ...
        {activeTab === "fraud" && (
          <div>
            ...
            {result && !loading && (
              <FraudPanel fraudResult={result.fraud_result} />
            )}
          </div>
        )}
        ...
        {activeTab === "underwriter" && (
          <div>
            ...
            {result && !loading && (
              <UnderwriterNotes
                verdictResult={result.final_verdict}
        ...
        {/* ── System Metrics (Architecture Notes) ────────────────────────── */}
        {activeTab === "verification" && (
          <div className="mt-4">
            <button
              onClick={async () => {
              ...
              <div className="mt-3 rounded-xl border border-gray-200 bg-white p-5">
                {metricsLoading && (
                  <p className="text-xs text-indigo-500 animate-pulse">
                    Running all 3 mock files to compute accuracy…
                  </p>
                )}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                      {[
                        ...
                        <div key={m.label} className="bg-gray-50 rounded-lg p-3 text-center border border-gray-100">
                          <p className="text-xl font-bold text-indigo-600">{m.value}</p>
